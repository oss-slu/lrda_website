...
const idNegotiation = function (resBody) {
  ...
  if (_contextid(resBody["@context"])) {
    ...
    modifiedResBody = Object.assign(context, { id: getConfig().rerum?.RERUM_ID_PREFIX + _id }, resBody);
  }
...
function getAgentClaim(req, next) {
  const claimKeys = [getConfig().rerum?.RERUM_AGENT_CLAIM, "http://devstore.rerum.io/v1/agent", "http://store.rerum.io/agent"];
  let agent = "";
...
function getAllDescendants(ls_versions, keyObj, discoveredDescendants) {
  ...
  for (let nextID of nextIDarr) {
    for (let v of ls_versions) {
      if (v["@id"] === nextID) {
        //If it is equal, add it to the known descendants
        //Recurse with what you have discovered so far and this object as the new keyObj
...
async function establishReleasesTree(releasing) {
  let success = true;
  const all = await getAllVersions(releasing).catch((error) => {
    console.error(error);
    return [];
  });
