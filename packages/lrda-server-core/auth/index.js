...
const devUser = (req, res, next) => {
  req.user = {
    ...
    name: 'Dev User',
    roles: ['admin'],
    'http://store.rerum.io/agent': 'dev-agent',
    'http://devstore.rerum.io/v1/agent': 'dev-agent'
  };
  next();
};
...
const checkJwt = (req, res, next) => {
  const disabled = getConfig().auth?.disableAuth === true;
  if (disabled) {
    return compose([READONLY, devUser])(req, res, next);
  }
  return compose([READONLY, auth(), _tokenError, _extractUser])(req, res, next);
};
