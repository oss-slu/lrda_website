      ...
      <ScrollArea className="flex-1 overflow-auto min-h-0">
        <div className="pb-20">
          ...
          {note.text && note.text.length > 0 ? (
            <div dangerouslySetInnerHTML={{ __html: data }} className="mb-5 note-content px-6" />
          ) : (
          ...
          {note.audio.length > 0 && (
            <Popover>
              <PopoverTrigger asChild>
                <div
                  className="flex flex-row shadow-sm rounded-full h-9 w-9 bg-white border border-border justify-center items-center
