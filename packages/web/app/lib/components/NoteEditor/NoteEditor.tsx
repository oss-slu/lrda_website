...
export default function NoteEditor({ note: initialNote, isNewNote, onNoteDeleted }: NoteEditorProps) {
  ...
  const showCommentSidebar = !!noteState.note?.id && canComment && isCommentSidebarOpen && (isViewingStudentNote || isStudentViewingOwnNote);
          ...
          >
            <ScrollArea className="flex flex-col w-full h-full min-h-0">
              <div aria-label="Top Bar" className="w-full flex flex-col px-8 pt-8">
                ...
                <div className="flex flex-row items-center gap-4 -mt-4 pb-4">
                  <NoteEditorToolbar
                    noteState={noteState}
                    noteHandlers={noteHandlers}
                    isViewingStudentNote={isViewingStudentNote}
