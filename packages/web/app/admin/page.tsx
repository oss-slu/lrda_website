...
export default function AdminDashboard() {
  ...
  const { isLoading: authLoading, isInitialized, isAdmin } = useAuthStore(
    useShallow((state) => ({
  ...
  // Check authorization - call isAdmin() since it's a function
  const userIsAdmin = isAdmin();

  // Redirect non-admins
  useEffect(() => {
    if (isInitialized && !authLoading && !userIsAdmin) {
      router.push("/");
    }
  }, [isInitialized, authLoading, userIsAdmin, router]);
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatsCard
            ...
            loading={isLoadingData}
            highlight={
              stats?.pendingApplications ? stats.pendingApplications > 0 : false
            }
          />
        </div>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This will reject the instructor application
                                      from {app.name}. They can reapply in the
                                      future.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This will approve {app.name} as an
                                      instructor. They will be able to manage
                                      students and review notes.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
    ...
    <Card className={highlight ? "border-orange-200 bg-orange-50" : ""}>
      <CardContent className="pt-6">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`rounded-full p-3 ${highlight ? "bg-orange-100" : "bg-gray-100"}`}
          >
            <Icon
              className={`h-5 w-5 ${highlight ? "text-orange-600" : "text-gray-600"}`}
            />
          </div>
        </div>
      </CardContent>
