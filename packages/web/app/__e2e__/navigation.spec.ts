...
test.describe('Navigation', () => {
  ...
  test('should display Resources page with properly formatted citations', async ({ page }) => {
    ...
    const citation1 = page.locator(
      'text=/American Anthropological Association Resources on Ethics/',
    );
    await expect(citation1).toBeVisible({ timeout: 10000 });
    // Verify it's NOT italicized (plain text)
    const italicSpan1 = citation1
      .locator('..')
  ...
  test('should navigate to correct URLs when clicking Online Resources links', async ({
    page,
    context,
  }) => {
    // Navigate to Resources page
