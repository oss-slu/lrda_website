...
test.describe('Admin to Instructor Application Flow', () => {
  ...
  test('should handle admin-to-instructor application page access', async ({ page }) => {
    ...
    const loadingText = page
      .locator('h1:has-text("Loading..."), .text-xl:has-text("Loading...")')
      .first();
    ...
    if (mainVisible) {
      ...
      await expect(
        page.locator(
          'text=Complete your instructor application using your existing admin information',
        ),
      )
  ...
  test('should handle application submission appropriately', async ({ page }) => {
    ...
    if (await descriptionField.isVisible()) {
      ...
      await descriptionField.fill(
        'I want to become an instructor to help students learn and grow.',
      );
  ...
  test('should handle important information section appropriately', async ({ page }) => {
    ...
    if (await importantInfo.isVisible()) {
      ...
      await expect(
        page.locator('text=Your application will be reviewed by an administrator'),
      ).toBeVisible();
      await expect(
        page.locator('text=You will be notified once your application is approved or rejected'),
      ).toBeVisible();
  ...
  test('should handle non-admin users appropriately', async ({ page }) => {
    ...
    // Check if we can access the page content (only if authenticated as admin)
    const nonAdminMessage = page.locator(
      'text=Only administrators can apply to become instructors',
    );
    const accessDeniedMessage = page.locator('text=Access denied');
    const homePageContent = page.locator("text=Where's Religion?");
...
test.describe('Real Application Behavior', () => {
  ...
  test('should show appropriate content for authenticated users', async ({ page }) => {
    ...
    if (pageContent?.includes('Login')) {
      ...
      pageContent?.includes('Apply to Become an Instructor') ||
      pageContent?.includes('Complete Your Instructor Profile')
    ) {
      // User sees the application form
      await expect(page.locator('form')).toBeVisible();
    } else if (
      pageContent?.includes('Access denied') ||
      pageContent?.includes('Admin privileges required')
    ) {
...
test.describe('Navbar Behavior', () => {
  test('should handle conditional instructor section on About page appropriately', async ({
    page,
  }) => {
