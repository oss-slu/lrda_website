 ...
 * /api/refreshToken:
 *   post:
 *     summary: Request a new refresh token (legacy compatibility route).
 *     description: |
 *       PUBLIC ENDPOINT.
 *
 *       Exchange a valid access token for a new refresh token using old compatibility route (for legacy clients). For new projects use `/request-new-refresh-token` route instead.
 *     tags:
 *       - Authentication
 *     requestBody:
 *       required: true
 ...
 *                 refreshToken:
 *                   type: string
 *                   example: "returnedRefreshToken..."
 *       '400':
 *         description: Missing or invalid access token.
 *       '401':
 *         description: Unauthorized/expired/blacklisted access token.
 */
