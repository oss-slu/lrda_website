...
jest.mock('app/lib/components/click_note_card', () => ({ __esModule: true, default: ({ note }: any) => (
  <div data-testid="note-card">{note.title || note.id}</div>
)}));
...
class IO {
  ...
  constructor(cb: IntersectionObserverCallback) { this.cb = cb; lastIO = this; }
  observe = (_el: Element) => {};
  unobserve = () => {};
  disconnect = () => {};
}
